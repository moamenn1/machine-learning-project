# System Architecture Overview

## 🏗️ Complete Pipeline Architecture

```
┌─────────────────────────────────────────────────────────────────┐
│                    INPUT: Raw Dataset Images                     │
│                    (1,960 images, 6 classes)                     │
└────────────────────────┬────────────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────────────┐
│              PHASE 1: Data Augmentation                          │
│              (data_augmentation.py)                              │
├─────────────────────────────────────────────────────────────────┤
│  Techniques:                                                     │
│  • Rotation (±20°)                                              │
│  • Horizontal Flip                                              │
│  • Brightness Adjustment (0.7-1.3x)                             │
│  • Scaling (0.8-1.2x)                                           │
│  • Gaussian Noise                                               │
├─────────────────────────────────────────────────────────────────┤
│  Output: 3,000 images (500 per class)                           │
└────────────────────────┬────────────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────────────┐
│              PHASE 2: Feature Extraction                         │
│              (feature_extraction.py)                             │
├─────────────────────────────────────────────────────────────────┤
│  Step 1: Preprocessing                                           │
│    • Resize to 128×128                                          │
│    • Gaussian Blur (3×3)                                        │
│                                                                  │
│  Step 2: HOG Features                                            │
│    • 9 orientations                                             │
│    • 8×8 pixels per cell                                        │
│    • 2×2 cells per block                                        │
│    • Output: ~1,700 features                                    │
│                                                                  │
│  Step 3: Color Histogram                                         │
│    • 32 bins per channel (BGR)                                  │
│    • L1 normalization                                           │
│    • Output: 96 features                                        │
│                                                                  │
│  Step 4: Concatenation                                           │
│    • Combined vector: ~1,800 features                           │
├─────────────────────────────────────────────────────────────────┤
│  Output: Feature matrix X (3000 × 1800)                         │
└────────────────────────┬────────────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────────────┐
│              PHASE 3: Feature Scaling                            │
│              (StandardScaler)                                    │
├─────────────────────────────────────────────────────────────────┤
│  • Z-score normalization: z = (x - μ) / σ                      │
│  • Fit on training set                                          │
│  • Transform train and test sets                                │
└────────────────────────┬────────────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────────────┐
│              PHASE 4: Train/Test Split                           │
├─────────────────────────────────────────────────────────────────┤
│  • Training: 80% (2,400 samples)                                │
│  • Validation: 20% (600 samples)                                │
│  • Stratified split (maintains class distribution)              │
└────────────┬────────────────────────────┬───────────────────────┘
             │                            │
             ▼                            ▼
┌────────────────────────┐  ┌────────────────────────────────────┐
│   SVM Classifier       │  │   k-NN Classifier                  │
│   (train_models.py)    │  │   (train_models.py)                │
├────────────────────────┤  ├────────────────────────────────────┤
│ Hyperparameters:       │  │ Hyperparameters:                   │
│ • Kernel: RBF          │  │ • k: 5                             │
│ • C: 10.0              │  │ • Weights: distance                │
│ • Gamma: scale         │  │ • Metric: euclidean                │
│ • Probability: True    │  │                                    │
├────────────────────────┤  ├────────────────────────────────────┤
│ Training:              │  │ Training:                          │
│ • Fit on X_train       │  │ • Store X_train, y_train           │
│ • Learn support vectors│  │ • No explicit training             │
│ • Optimize decision    │  │   (lazy learning)                  │
│   boundaries           │  │                                    │
├────────────────────────┤  ├────────────────────────────────────┤
│ Prediction:            │  │ Prediction:                        │
│ • Fast (support        │  │ • Compute distances to all         │
│   vectors only)        │  │   training samples                 │
│ • Probability output   │  │ • Find k nearest neighbors         │
│                        │  │ • Distance-weighted vote           │
├────────────────────────┤  ├────────────────────────────────────┤
│ Output:                │  │ Output:                            │
│ svm_classifier.pkl     │  │ knn_classifier.pkl                 │
└────────────┬───────────┘  └────────────┬───────────────────────┘
             │                           │
             └───────────┬───────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────────────┐
│              PHASE 5: Model Evaluation                           │
│              (evaluate_model.py, compare_models.py)              │
├─────────────────────────────────────────────────────────────────┤
│  Metrics:                                                        │
│  • Accuracy                                                      │
│  • Precision, Recall, F1-score (per class)                      │
│  • Confusion Matrix                                              │
│  • Prediction Speed                                              │
│  • Memory Usage                                                  │
└────────────────────────┬────────────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────────────┐
│              PHASE 6: Real-time Deployment                       │
│              (realtime_classifier.py)                            │
├─────────────────────────────────────────────────────────────────┤
│  Pipeline:                                                       │
│                                                                  │
│  Camera Frame                                                    │
│      │                                                           │
│      ▼                                                           │
│  Preprocess (resize, blur)                                       │
│      │                                                           │
│      ▼                                                           │
│  Extract Features (HOG + Color)                                  │
│      │                                                           │
│      ▼                                                           │
│  Scale Features (StandardScaler)                                 │
│      │                                                           │
│      ▼                                                           │
│  Predict (SVM or k-NN)                                          │
│      │                                                           │
│      ▼                                                           │
│  Rejection Check (confidence < 0.6?)                            │
│      │                                                           │
│      ├─ Yes → Classify as "unknown"                            │
│      └─ No  → Use predicted class                              │
│      │                                                           │
│      ▼                                                           │
│  Display Result (class + confidence)                             │
│                                                                  │
├─────────────────────────────────────────────────────────────────┤
│  Features:                                                       │
│  • Real-time processing (10-30 FPS)                             │
│  • Confidence visualization                                      │
│  • Color-coded classes                                          │
│  • Model switching (press 's')                                  │
└─────────────────────────────────────────────────────────────────┘
```

## 📊 Data Flow Diagram

```
Raw Image (H×W×3)
    │
    ├─→ Preprocessing
    │       │
    │       ├─→ Resize to 128×128
    │       └─→ Gaussian Blur
    │
    ├─→ HOG Extraction
    │       │
    │       ├─→ Convert to Grayscale
    │       ├─→ Compute Gradients
    │       ├─→ Orientation Binning
    │       └─→ Block Normalization
    │       │
    │       └─→ HOG Vector (~1,700 dims)
    │
    ├─→ Color Histogram
    │       │
    │       ├─→ For each channel (B, G, R):
    │       │       ├─→ Compute histogram (32 bins)
    │       │       └─→ Normalize
    │       │
    │       └─→ Color Vector (96 dims)
    │
    └─→ Concatenate
            │
            └─→ Feature Vector (~1,800 dims)
                    │
                    ├─→ Scale (StandardScaler)
                    │
                    └─→ Classify (SVM or k-NN)
                            │
                            └─→ Class Label + Confidence
```

## 🔄 Training vs Inference

### Training Mode (train_models.py)

```
Dataset (3,000 images)
    │
    ├─→ Extract features for all images
    │       │
    │       └─→ Feature matrix X (3000 × 1800)
    │
    ├─→ Split train/test (80/20)
    │       │
    │       ├─→ X_train (2400 × 1800)
    │       └─→ X_test (600 × 1800)
    │
    ├─→ Fit scaler on X_train
    │       │
    │       └─→ Save scaler.pkl
    │
    ├─→ Transform X_train and X_test
    │
    ├─→ Train SVM
    │       │
    │       └─→ Save svm_classifier.pkl
    │
    └─→ Train k-NN
            │
            └─→ Save knn_classifier.pkl
```

### Inference Mode (realtime_classifier.py)

```
New Image
    │
    ├─→ Extract features
    │       │
    │       └─→ Feature vector (1 × 1800)
    │
    ├─→ Load scaler.pkl
    │       │
    │       └─→ Transform features
    │
    ├─→ Load model.pkl (SVM or k-NN)
    │       │
    │       └─→ Predict class + confidence
    │
    └─→ Apply rejection threshold
            │
            ├─→ confidence < 0.6 → "unknown"
            └─→ confidence ≥ 0.6 → predicted class
```

## 🎯 Class Distribution

### Before Augmentation
```
Glass     ████████████████████████████████████████ 401
Paper     ████████████████████████████████████████████████ 476
Cardboard ██████████████████████████ 259
Plastic   ████████████████████████████████████ 386
Metal     ████████████████████████████████ 328
Trash     ███████████ 110
```

### After Augmentation
```
Glass     ██████████████████████████████████████████████████ 500
Paper     ██████████████████████████████████████████████████ 500
Cardboard ██████████████████████████████████████████████████ 500
Plastic   ██████████████████████████████████████████████████ 500
Metal     ██████████████████████████████████████████████████ 500
Trash     ██████████████████████████████████████████████████ 500
```

## 🧠 Model Architectures

### SVM Architecture
```
Input Features (1800 dims)
    │
    ├─→ RBF Kernel Transformation
    │       │
    │       └─→ K(x, x') = exp(-γ ||x - x'||²)
    │
    ├─→ Decision Function
    │       │
    │       └─→ f(x) = Σ αᵢ yᵢ K(xᵢ, x) + b
    │
    ├─→ Multi-class (One-vs-Rest)
    │       │
    │       ├─→ 6 binary classifiers
    │       └─→ Softmax for probabilities
    │
    └─→ Output
            │
            ├─→ Class (0-5)
            └─→ Confidence (0-1)
```

### k-NN Architecture
```
Input Features (1800 dims)
    │
    ├─→ Compute Distances
    │       │
    │       └─→ d(x, xᵢ) = ||x - xᵢ||₂
    │
    ├─→ Find k=5 Nearest Neighbors
    │       │
    │       └─→ Sort by distance
    │
    ├─→ Distance-weighted Voting
    │       │
    │       └─→ weight = 1 / distance
    │
    └─→ Output
            │
            ├─→ Class (majority vote)
            └─→ Confidence (from distances)
```

## 📁 File Dependencies

```
config.py
    │
    ├─→ data_augmentation.py
    │       │
    │       └─→ Creates: dataset_augmented/
    │
    ├─→ feature_extraction.py
    │       │
    │       ├─→ Used by: train_models.py
    │       ├─→ Used by: realtime_classifier.py
    │       └─→ Used by: evaluate_model.py
    │
    ├─→ train_models.py
    │       │
    │       ├─→ Requires: feature_extraction.py
    │       └─→ Creates: models/*.pkl
    │
    ├─→ realtime_classifier.py
    │       │
    │       ├─→ Requires: feature_extraction.py
    │       └─→ Requires: models/*.pkl
    │
    ├─→ evaluate_model.py
    │       │
    │       ├─→ Requires: feature_extraction.py
    │       └─→ Requires: models/*.pkl
    │
    └─→ compare_models.py
            │
            ├─→ Requires: feature_extraction.py
            └─→ Requires: models/*.pkl
```

## 🚀 Execution Flow

### Option 1: Automated (run_pipeline.py)
```
run_pipeline.py
    │
    ├─→ Step 1: data_augmentation.py
    │       └─→ Creates balanced dataset
    │
    ├─→ Step 2: train_models.py
    │       └─→ Trains and saves models
    │
    └─→ Step 3: realtime_classifier.py
            └─→ Launches demo
```

### Option 2: Manual
```
1. python data_augmentation.py
2. python train_models.py
3. python compare_models.py
4. python realtime_classifier.py svm
```

## 💾 Saved Artifacts

```
models/
    ├─→ svm_classifier.pkl      (SVM model, ~1-5 MB)
    ├─→ knn_classifier.pkl      (k-NN model, ~10-50 MB)
    └─→ feature_scaler.pkl      (StandardScaler, ~100 KB)

dataset_augmented/
    ├─→ glass/      (500 images)
    ├─→ paper/      (500 images)
    ├─→ cardboard/  (500 images)
    ├─→ plastic/    (500 images)
    ├─→ metal/      (500 images)
    └─→ trash/      (500 images)
```

## 🎯 Key Design Decisions

| Decision | Rationale |
|----------|-----------|
| HOG + Color features | Combines shape and appearance |
| RBF kernel for SVM | Handles non-linear boundaries |
| k=5 for k-NN | Balances noise sensitivity |
| Distance weighting | Closer neighbors more important |
| 128×128 image size | Balance between detail and speed |
| 0.6 confidence threshold | Balances precision and recall |
| StandardScaler | Essential for distance-based methods |
| 80/20 train/test split | Standard practice |

---

This architecture achieves the project goals while maintaining simplicity and efficiency.
